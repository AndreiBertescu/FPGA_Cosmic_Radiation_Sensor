...
always @(posedge clk or posedge reset_i) begin
    if(reset_i)
        shift_reg_tx <= #TP {9{1'b1}};
    else if(send_data_posedge_baud & baud_clk)
        shift_reg_tx <= #TP (LSB_FIRST) ? {1'b0, data_tx_i} : {data_tx_i, 1'b0};
    else if(baud_clk)
        shift_reg_tx <= #TP (LSB_FIRST) ? {shift_reg_tx[7 : 0], 1'b1} : {1'b1, shift_reg_tx[8 : 1]};  
end

assign tx = (LSB_FIRST) ? shift_reg_tx[8] : shift_reg_tx[0];
...
always @(posedge clk or posedge reset_i) begin
    ...
    else if(counter_tx == 4'd10 & baud_clk)
        sending_data <= #TP 1'b0;
...
always @(posedge clk or posedge reset_i) begin
    if(reset_i)
        shift_reg_rx <= #TP {9{1'b0}};
    else if(baud_clk)
        shift_reg_rx <= #TP (LSB_FIRST) ? {shift_reg_rx[7 : 0], rx} : {rx, shift_reg_rx[8 : 1]};  
end
...
always @(posedge clk or posedge reset_i) begin
    if(reset_i)
        data_rx_o <= #TP {8{1'b0}};
    else if(counter_rx == 4'd8 & baud_clk)
